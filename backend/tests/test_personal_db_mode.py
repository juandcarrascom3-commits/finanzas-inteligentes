...
def test_sqlite_migrations_are_versioned_and_idempotent(tmp_path, monkeypatch):
    ...
    assert first_schema["latest_version"] == "007"
    assert [item["version"] for item in first_schema["migrations"]] == ["001", "002", "003", "004", "005", "006", "007"]
...
def test_backup_validate_and_restore_roundtrip(tmp_path, monkeypatch):
    ...
    assert validation["latest_version"] == "007"
